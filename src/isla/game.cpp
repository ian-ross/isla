...
class IslaCellBox {
public:
  bool IsAlive(int dx, int dy) const;
  bool SetCell(int dx, int dy, bool alive);
  ...
  IslaCellBox  *m_up, *m_dn, *m_lf, *m_rt;    // neighbour CellBoxes
  IslaCellBox  *m_prev, *m_next;              // in linked list
  IslaCellBox  *m_hprev, *m_hnext;            // in hash table
};
...
//
bool IslaCellBox::IsAlive(int dx, int dy) const
{
  if (dy > 3)
...
//
bool IslaCellBox::SetCell(int dx, int dy, bool alive)
{
...
Isla::Isla()
{
  // pattern data
  m_numcells    = 0;
  m_boxes       = new IslaCellBox *[HASHSIZE];
  m_head        = NULL;
  m_available   = NULL;
  for (int i = 0; i < HASHSIZE; i++) m_boxes[i] = NULL;

  // state vars for BeginFind & FindMore
  m_cells       = new IslaCell[ARRAYSIZE];
  m_ncells      = 0;
  m_findmore    = false;
  m_changed     = false;
}
...
//
bool Isla::IsAlive(wxInt32 x, wxInt32 y)
{
  IslaCellBox *c = LinkBox(x, y, false);
...
//
void Isla::SetCell(wxInt32 x, wxInt32 y, bool alive)
{
  IslaCellBox *c  = LinkBox(x, y);
  wxUint32 dx = x - c->m_x;
  wxUint32 dy = y - c->m_y;
...
//
IslaCellBox* Isla::CreateBox(wxInt32 x, wxInt32 y, wxUint32 hv)
{
    IslaCellBox *c;
    ...
    // reset everything
    memset((void *) c, 0, sizeof(IslaCellBox));
    c->m_x = x;
    c->m_y = y;
...
//
IslaCellBox* Isla::LinkBox(wxInt32 x, wxInt32 y, bool create)
{
    wxUint32 hv;
    IslaCellBox *c;
    ...
    return create? CreateBox(x, y, hv) : (IslaCellBox*) NULL;
}
...
//
void Isla::KillBox(IslaCellBox *c)
{
...
//
void Isla::DoLine(wxInt32 x, wxInt32 y, wxUint32 live, wxUint32 old)
{
...
void Isla::BeginFind(wxInt32 x0, wxInt32 y0, wxInt32 x1, wxInt32 y1, bool changed)
{
...
bool Isla::FindMore(IslaCell *cells[], size_t *ncells)
{
    IslaCellBox *c;
    *cells = m_cells;
    m_ncells = 0;
...
class IslaModule: public wxModule
{
DECLARE_DYNAMIC_CLASS(IslaModule)
...
IMPLEMENT_DYNAMIC_CLASS(IslaModule, wxModule)

bool IslaModule::OnInit()
{
    // see below
